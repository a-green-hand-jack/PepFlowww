...
from pepflow.utils.misc import (
    BlackHole,
    inf_iterator,
    load_config,
    seed_all,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", type=str, default="./configs/angle/learn_angle.yaml"
    )
    ...
    if args.debug:
        ...
        run = wandb.init(
            project=args.name, config=config, name="%s[%s]" % (config_name, args.tag)
        )
        if args.resume:
            ...
            log_dir = get_new_log_dir(
                args.logdir,
                prefix="%s[%s]" % (config_name, version_short),
                tag=args.tag,
            )
    ...
    def validate(it):
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(
                ...
                loss = sum_weighted_losses(loss_dict, config.train.loss_weights)
                scalar_accum.add(
                    name="loss", value=loss, batchsize=len(batch["aa"]), mode="mean"
                )
                for k, v in loss_dict["scalar"].items():
                    scalar_accum.add(
                        name=k, value=v, batchsize=len(batch["aa"]), mode="mean"
                    )
